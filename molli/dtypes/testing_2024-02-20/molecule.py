...
class Molecule:
    ...
    def join(
        ...
        try: ####### currently held together with duct tape... if it is necessary, fucking fix this later.
            ...
            #currently, the following process adds all atoms that were obtained from the "recursive_atoms()" call.
            #in some cases, e.g., with cyclohexyl, there are protons that would be added to this list. These protons
            #should not contribute to the plane calculation and should be removed. So, only ensure that carbon atoms are kept    
            ...
            #here, let's delete all non-carbon containing atoms from the list

            for atoms in a1list:
                a1_geoms.append(g1.coord[m1.get_atom_idx(atoms)].tolist())
            for atoms in a2list:
                print(atoms)
                if(atoms.label == "C"): ## this if-statement provides the "carbon atom" check. Ensure it has the correct behaviour before leaving it in here...
            ...
            for i,rotation in enumerate(scan):
                ...
                ####
                # the following line generates rotated coordinates by selecting the first two atoms of the alkyne fragment.
                # the axis of rotation is determined by the first two coordinates. There should be a better way to do this
                # the "joined" atoms and the next atom in line are the better options.
                
                #_tmpcoord = sub_rotate_around_line(g2.coord, g2.coord[0],g2.coord[1], float(scan[i])) ## this line is problematic if the first atom is NOT the alkyne carbon
                # let's make sure we use the alkyne carbons....
                # in principle, the "recursive_atoms()" call should give us the two carbon atoms we are interested in.
                _tmpcoord = sub_rotate_around_line(g2.coord, np.array(a2_geoms[0]), np.array(a2_geoms[1]),float(scan[i])) #need to pass a numpy array to do the math on the other side...
