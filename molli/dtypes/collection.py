...
class Collection:
    ...
    # modification by KSP (2023-08-15)
    # in some cases it is valuable to not do a combinatorial enumeration (e.g. when two identical
    # fragments should be joined to the same atom). So, use zip() instead
    # added "jointype" variable, that is set to "product" as default, which was the original
    # method that was implemented
    """
    other features to add:
    - join to centroid of n atoms (e.g., move to centroid of bond when n=2)
    - join two pieces at once (make rings, useful for bidentate ligands)
    ...
    def join(
        ...
        molecules = []
        # modification by KSP (2023-08-15)
        # in some cases it is valuable to not do a combinatorial enumeration (e.g. when two identical fragments should be joined to the same atom)
        # so just combine them

        if(jointype == "product"):
            for m1, m2 in product(mc1, mc2):
                #print("JOINING BY PRODUCT TYPE", m1.name, m2.name, ap1, ap2)

                #x=input()
                molecules.append(Molecule.join_ap(m1, m2, ap1=ap1, ap2=ap2, dist=dist, addname=addname, rotations = rotations, central_atom = central_atom, scan = "Normal"))
                m1.fragment_atoms = []
        elif(jointype == "zip"):
            for m1, m2 in zip(mc1, mc2):

                #x=input("examining collections")
                
                #print("JOINING BY ZIP TYPE", m1.name, m2.name, ap1, ap2)
                #x=input()
                molecules.append(Molecule.join_ap(m1, m2, ap1=ap1, ap2=ap2, dist=dist, addname=addname, rotations = rotations, central_atom = central_atom, scan = "Normal"))
                m1.fragment_atoms = []
        

        #print("here now", molecules[25].atoms[0]==mc1[0].atoms[0], molecules[25].bonds[0]==mc1[0].bonds[0], f"\n {molecules[0].bonds} ------\n------ {mc1[0].bonds}", len(molecules[0].atoms), len(mc1[0].atoms) )
        #print("\n\n\n\n\n\n OK CHECKING HERE NOW, FCK")
        #x=input("goddamnit")
