...
SUPPORTED_EXTENSIONS = set(('out', 'log', 'sdf', 'xyz', 'pdb', 'sdf','csv'))
...
class calcD3:
   def __init__(self, fileData, functional, damp='zero', s6=0.0, rs6=0.0, s8=0.0, a1=0.0, a2=0.0, abc=False, intermolecular=False, pairwise=False, verbose=False, xyzdata = [], base_struct=[], fragments = [], name = []):

      try:
          atom_nums = fileData.atomnos.tolist()
          cartesians = fileData.atomcoords[-1].tolist()
          atomtype = [periodictable[atno] for atno in atom_nums] # contains atom types as numbers
      except:
          print("File data is an .xyz file")
          #atom_nums = xyzdata[0]
          cartesians = xyzdata[1].squeeze().tolist()
          atomtype = xyzdata[0] # contains atom types as letters
      
      self.funct_list_run = []
      self.frag_disp_data = frag_disp_data
      natom = len(atomtype)
                   ...
                   if(len(base_struct)>0):
                       if j in base_struct:
                           if k not in base_struct:
                               disp_sum = disp_sum + self.attractive_r6_term + self.attractive_r8_term
                               #print("   --- base_struct pairwise interactions", (j+1), "and", (k+1),": Edisp =", "%.6f" % (self.attractive_r6_term + self.attractive_r8_term), "kcal/mol")
                   if(len(fragments)>0):

                       #print(fragments[0],fragments[1],fragments[2])
                       if j in fragments[0] and j<(int(fragments[0][-1])-36) : # I forgot where the 36 came from... omg comment all code....
                           #print(fragments[0])
                           if k not in fragments[0] and k not in fragments[2] :
                           
                               
                               #print("F1-F2, checking: ", cartesians[j], cartesians[k])
                               #print("SUM 1: j,k", j, k, disp_sum1, self.attractive_r6_term, self.attractive_r8_term, "distance: ", totdist)
                               disp_sum1 = disp_sum1 + self.attractive_r6_term + self.attractive_r8_term
                               #print("disp_sum1", disp_sum1)
                           if k not in fragments[0] and k not in fragments[1]:
                               #print("j,k", j, k, self.attractive_r6_term, self.attractive_r8_term)
                               #print("---------------------------------------------------------------F1-F3, checking: ", cartesians[j], cartesians[k])
                               #print("SUM 2: j,k", j, k, disp_sum2, self.attractive_r6_term, self.attractive_r8_term, "distance: ", totdist)
                               disp_sum2 = disp_sum2 + self.attractive_r6_term + self.attractive_r8_term
      ...
      try:
          _tmp = [name, disp_sum1, disp_sum2]
          self.frag_disp_data.append(_tmp)
          print("DISPERSION SUM BETWEEN F1-F2: ", disp_sum1)
          print("DISPERSION SUM BETWEEN F1-F3: ", disp_sum2)
          print("TOTAL:", disp_sum)
      except:
          print("DISPERSION SUM BETWEEN FRAGMENTS:", disp_sum)
      _tmp = [functional,disp_sum]
      self.funct_list_run.append([functional,disp_sum])
...
def main():
    ...
    parser.add_argument("--func", dest="functional", default="B3LYP",
    ...
    parser.add_argument("--xyzinp", dest="xyzinp", type=str, default=False)
    parser.add_argument("--base", dest="base", type=list_of_int, default=False)
    parser.add_argument("--base_file", dest="base_file", type=str, default=False)
    parser.add_argument("--fragments", dest="fragments", type=str, default=False)
    parser.add_argument("--iterate_func", dest="iterate_func", type=bool, default=False)
    ...
    try:
        with open(options.base_file, "r+") as f:
            options.base = f.readline()[:-1]#list_of_int((f.readline()))
            _tmp = list_of_int(options.base)
    except:
        pass
    ...
    if(options.fragments): ## the fragments file should contain a list of the xyz file names...
        frag_list = []
        fragments_df = pd.read_csv(options.fragments, header=None, index_col = None) #open the data. The first row should contain information about the xyz file
        frag_files = len(fragments_df.to_numpy())-1
        fragments_np = fragments_df.to_numpy()
        #print(fragments_np)
        #print(fragments_np[1])
        labels = []
        for label in fragments_np[1][1:]:
            #print(label)
        ...
        current_labels = [] #contains list of atom numbers for each fragment (separated by lists)


        for i in range(1, frag_files+1):
            #print(f"CHECKING: {i}, {fragments_np[i][1]}")
            #with open(str(fragments_np[i][1])) as f:
            xyz_df = pd.read_csv(f"{fragments_np[i][1]}.xyz", delim_whitespace=True, names = ["atoms", "x", "y", "z"], skiprows=[0, 1])
            xyz_df = xyz_df.reset_index(drop=True)
            atomlist = xyz_df["atoms"].tolist()
            xyz_df.drop("atoms", axis=1, inplace=True)            
            cartesians = xyz_df.to_numpy().squeeze()
            _tmp = []
            _xyzfile = []
            #print("labels", labels)
            #print(f"opening {fragments_np[i][1]}.xyz")
            #print(len(atomlist), len(fragments_np[frag_files][1:]))
            #print(fragments_np)
            #exit()
            for k,label in enumerate(labels):
                for j,item in enumerate(fragments_np[i][1:]):
                    if str(label) == item:
                        _tmp.append(j-1)
                        #print("for: ", label, " : ", j)
                _xyzfile.append(_tmp)
                _tmp = []

            
            data=None
            #print(f"sending: {fragments_np[i][1]}.xyz")
            #lets quickly validate that the fragments are chosen correctly....
            #for set in _xyzfile
            
            #print(atomlist, len(atomlist))
            #print(cartesians, len(cartesians))
            
            """
            for num, fragment_number in enumerate(_xyzfile):
                #print(fragment_number)
                #print(fragments_np[i][1])
                filename = f"{fragments_np[i][1]}_frag_{num}.xyz"
                with open(filename, "w+") as f:
                    for number in fragment_number:
                        pass
                        #print("writing: ", number)
                        f.write(f"{atomlist[number]} {cartesians[number][0]} {cartesians[number][1]} {cartesians[number][2]} \n")
            """

            print("SENDING: ", fragments_np[i][1])
            #print(_xyzfile)
            #exit()


            fileD3 = calcD3(data, dft_functional, options.damp, options.s6, options.rs6, options.s8, options.a1, options.a2, options.threebody, \
              options.intermolecular, options.pairwise, options.verbose, xyzdata = [atomlist,cartesians], fragments = _xyzfile, name = fragments_np[i][1])
            #exit()
            current_labels.append(_xyzfile)
        ...
        frag_disp_data_df = pd.DataFrame(frag_disp_data)
        frag_disp_data_df.to_csv(f"{options.fragments}_dispersion_data.csv")
    ...
    for file in files:
        ...
        # inputting an xyz file instead
        if(options.xyzinp):
            xyz_df = pd.read_csv(options.xyzinp, delim_whitespace=True, names = ["atoms", "x", "y", "z"], skiprows=[0, 1])
            xyz_df = xyz_df.reset_index(drop=True)
            ...
            data=None
            disp_funct_data = []
            if(options.iterate_func):
                for functional in FUNC_LIST:
                    print("SENDING", functional)
                    fileD3 = calcD3(data, functional, options.damp, options.s6, options.rs6, options.s8, options.a1, options.a2, options.threebody, \
                    options.intermolecular, options.pairwise, options.verbose, xyzdata = [atomlist,cartesians], base_struct = _tmp)
                    disp_funct_data.append(fileD3.funct_list_run[0])
                    print(disp_funct_data)
                    del fileD3

                    
                np1 = np.array(disp_funct_data)
                pd.DataFrame(np1).to_csv(f"DISP_FOR_XYZ_{options.xyzinp}.csv")
            else:
                fileD3 = calcD3(data, dft_functional, options.damp, options.s6, options.rs6, options.s8, options.a1, options.a2, options.threebody, \
                options.intermolecular, options.pairwise, options.verbose, xyzdata = [atomlist,cartesians], base_struct = _tmp)              
